`timescale 1ns/1ps
// ===================================================================
// tb_comp_rle.v -- Self-checking testbench for comp_rle
// -------------------------------------------------------------------
//   1. Nap tb_src_mixed.hex            -> mem[0..4095]   (source)
//   2. Nap tb_expected_rle_mixed.hex   -> exp[]          (expected dest)
//   3. Loop over 256 blocks: start -> wait done -> accumulate dest_base
//   4. Compare mem[DEST_BASE+k] with exp[k]; print PASS/FAIL
//
// Run (iverilog):
//   iverilog -o tb_rle comp_rle.v tb_comp_rle.v && vvp tb_rle
// Run (ModelSim):
//   vlog comp_rle.v tb_comp_rle.v
//   vsim -c tb_comp_rle -do "run -all; quit"
// ===================================================================
module tb_comp_rle;
    localparam AW = 14;                // RLE co the no ra: dest can > 8191
    localparam N  = 16;
    localparam MEMSZ     = 16384;
    localparam NUM_BLK   = 256;
    localparam SRC_BASE  = 0;
    localparam DEST_BASE = 4096;
    localparam EXP_LEN   = 4318;        // tu golden_compress.py (mode rle)
    ...
    initial begin
        ...
        $readmemh("tb_expected_rle_mixed.hex", exp);
        ...
        if ((cur_dest-DEST_BASE)==EXP_LEN && errors==0)
            $display("  [PASS] comp_rle matches golden model over all %0d blocks.", NUM_BLK);
        else
